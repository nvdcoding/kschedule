...
import {
  validateEmail,
  validatePhone,
} from 'src/base/utils/ValidationUtils';
import {
  Block,
  Image,
  Spinner,
...
const defaultRegister: IDataSignUp = {
  firstName: '',
  lastName: '',
  email: '',
  password: '',
...
const RegisterScreen = ({navigation}) => {
  ...
  const [dataSignUp, setDataSignUp] = useState<IDataSignUp>(defaultRegister);
  ...
  const handleSignUp = async () => {
    ...
    try {
      Object.keys(dataSignUp).forEach((item, index) => {
        ...
        if (index === 5 && !validatePhone(dataSignUp[item])) {
          setInvalid(index);
          throw t('PHONE_NUMBER_INVALID');
        }
      });
      setLoading(true);
